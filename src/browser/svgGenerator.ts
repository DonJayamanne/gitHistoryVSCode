...
function drawGitGraph(startAt, logEntryHeight: number = 60.8, entries: contracts.LogEntry[]) {
    ...
    for (let i = startAt; i < content.children.length; ++i) {
        ...
        let index = 0;
        (entryElement as any).webuiLeft = branches.length;
        ...
        if (padParentCount > 0) {
            for (let parentElemtnCounter = startAt; parentElemtnCounter <= i; parentElemtnCounter++) {
                ...
                (el as any).webuiLeft += padParentCount;
            }
        }
    }
    for (let i = startAt; i < content.children.length; ++i) {
